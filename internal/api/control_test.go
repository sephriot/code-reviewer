...
func TestControlServesReadOnlyDashboard(t *testing.T) {
	...
	for _, text := range []string{"Control Desk", "/api/v1/inbox", "timeline", "selected-facts", "/api/v1/mutate/pull-requests", "Build canonical evidence", "Queue eligible review", "Review setup", "REVIEWD_REVIEW_ENGINE_ARGV", "/api/v1/notification-preferences", "Save local preferences"} {
